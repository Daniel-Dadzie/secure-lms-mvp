...
* **Refresh Tokens:** Configured with a 7-day lifespan. They are transmitted using secure, HTTP-only cookies (`withCredentials: true`) to prevent client-side script access.
...
When a user logs out, the backend explicitly clears the authentication cookies and invalidates the active refresh token family, ensuring immediate session termination.