...
export function Sidebar({
  ...
  function renderSidebarContent(isCollapsed: boolean, showCollapseToggle: boolean) {
    return (
      <div
        className={`flex h-full ${isCollapsed ? "w-[4.5rem]" : "w-64"} shrink-0 flex-col bg-[#0A4A3A] text-white transition-all duration-200`}
      >
        <div
          className={`flex items-center border-b border-[#196A54]/50 ${
            isCollapsed ? "justify-center px-2 py-4" : "gap-3 px-5 py-5"
          }`}
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#F4F9F7] text-[#0A4A3A]">
          ...
          {!isCollapsed && (
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-teal-100/60 font-semibold uppercase tracking-wider">
                Technologies LMS
              </p>
            </div>
          )}
        ...
        <nav className={`flex-1 space-y-1 py-4 overflow-y-auto ${isCollapsed ? "px-2" : "px-3"}`}>
          {navItems.map((item) => {
            const isActive =
              item.href === "/student" ||
                ...
                <div className={`flex items-center ${isCollapsed ? "" : "gap-3"}`}>
                  <span
                    className={`relative flex h-5 w-5 items-center justify-center ${
                      isActive
                        ? "text-white"
                        : "text-teal-100/50 group-hover:text-white transition-colors"
                    }`}
                  >
                    {item.icon}
        ...
        <div className="mt-auto border-t border-[#196A54]/50 p-3 space-y-2">
          {showCollapseToggle && onToggleCollapse && (
            <button
              type="button"
              onClick={onToggleCollapse}
              className={`hidden md:flex w-full items-center rounded-lg px-3 py-2 text-sm font-semibold text-teal-100/70 transition-colors hover:bg-[#196A54]/40 hover:text-white ${
          ...
          <button
            ...
            className={`flex w-full items-center rounded-lg px-3 py-2 text-sm font-semibold text-teal-100/70 transition-colors hover:bg-red-500/10 hover:text-red-400 ${
              isCollapsed ? "justify-center" : "gap-3"
            }`}
          ...
          <Link
            ...
            className={`flex items-center rounded-xl bg-[#196A54]/30 border border-[#196A54]/40 cursor-pointer hover:bg-[#196A54]/50 transition-colors ${
          ...
          >
            ...
            {!isCollapsed && (
              <>
                ...
                <svg
                  className="w-4 h-4 text-teal-100/40 shrink-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
